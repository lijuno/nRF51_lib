...
static uint32_t nrf_pwm_set_mode(nrf_pwm_config_t *config)
{ 
	  switch(config->mode)
    {
        ...
        case PWM_MODE_BUZZER_255:  // 8-bit resolution, 62.5kHz PWM frequency, 16MHz timer frequency (prescaler 0)
            ...
            pwm_max_value = 255;
            break;
		case PWM_MODE_125kHz:    // 125 kHz clock
	        PWM_TIMER->PRESCALER = 0;
            pwm_max_value = 127;
            break;	
		case PWM_MODE_250kHz:    // 250 kHz clock
	        PWM_TIMER->PRESCALER = 0;
            pwm_max_value = 63;
            break;	
		case PWM_MODE_500kHz:    // 500 kHz clock
	        PWM_TIMER->PRESCALER = 0;
            pwm_max_value = 31;
            break;		
		case PWM_MODE_1MHz:    // 1MHz clock
	        PWM_TIMER->PRESCALER = 0;
            pwm_max_value = 15;
            break;				
		case PWM_MODE_2MHz:    // 2MHz clock
	        PWM_TIMER->PRESCALER = 0;
            pwm_max_value = 7;
            break;	
        case PWM_MODE_4MHz:    // 4MHz clock
            PWM_TIMER->PRESCALER = 0;
            pwm_max_value = 3;
            break;	
        default:
...
static void nrf_pwm_timer_ppi_init(nrf_pwm_config_t *config)
{
...
uint32_t nrf_pwm_init(nrf_pwm_config_t *config)
{
  ...
  if (nrf_pwm_set_mode(config)!=0) return 0xFFFFFFFF;
	
	nrf_pwm_timer_ppi_init(config);
...
static void nrf_pwm_set_value_directly(uint32_t pwm_channel, uint32_t pwm_value)
{
	// A workaround for high PWM frequency (>62.5 kHz) https://devzone.nordicsemi.com/question/42518/random-pwm-dutycycle-inversion-in-softdevice-80/
	pwm_next_value[pwm_channel] = pwm_value;
	pwm_modified[pwm_channel] = true;
	PWM_IRQHandler();
}

uint32_t nrf_pwm_init2(nrf_pwm_config_t *config, uint32_t pwm_value)
{
	// A workaround for high PWM frequency (>62.5 kHz) https://devzone.nordicsemi.com/question/42518/random-pwm-dutycycle-inversion-in-softdevice-80/
    // Note that it assumes that there is only one channel
	// The rest is the same to nrf_pwm_init()
	
  if(config->num_channels !=1) return 0xFFFFFFFF;  // Only one channel is supported for now; can be easily modified for multi-channel in the future
  // if(config->num_channels == 0 || config->num_channels > PWM_MAX_CHANNELS) return 0xFFFFFFFF;
  ...
  if (nrf_pwm_set_mode(config)!=0) return 0xFFFFFFFF;
    ...
    NVIC_EnableIRQ(PWM_IRQn);
#endif
	nrf_pwm_set_value_directly(0, pwm_value);  // Init PWM duty cycle before starting PWM timer
	apply_pan73_workaround(PWM_TIMER, true);
	PWM_TIMER->TASKS_START = 1;
	if(pwm_num_channels > 2)
	{
...
void nrf_pwm_set_enabled(bool enabled)
{
    ...
    {   // The original code couldn't completely turn off PWM
		// Refer to post for workarounds http://bit.ly/1QOstpw
        ...
        if(pwm_num_channels > 2) 
				{
					...
					apply_pan73_workaround(PWM_TIMER2, false);  
					PWM_TIMER2->TASKS_SHUTDOWN = 1;
				}
